 ...
 * This is rendering quite alot of boxes in an unoptimized way. Speed was not important for this project
 * 
...
public class App extends PApplet
{
	...
	static String WINDOW_TITLE = "/CLNUIPinboxes";
	...
	public void draw()
	{
    	...
    	camera( cos(time*0.45f)*400, 200, 500, 0, 0, 0, 0, 1, 0 );
    	...
    	for( int j=0; j<kinect.getHeight(); j+=step )
    	{
        	...
        	{
        		p.z = kinect.getDepthAt( i, j ) / 65536.0f;
        		p.z = p.z*p.z;
        		p.z *= 400;
        		if( p.z > 20 && p.z < 250 )
        		{
	        		p.x = (i-kinect.getWidth()*0.5f) * 0.5f;
	        		p.y = (j-kinect.getHeight()*0.5f) * 0.5f;
	        		//p.z = kinect.getDistanceAt( i, j );
	        		//p.z = kinect.getNormDistanceAt( i, j ) * 400;
	        		//point( p.x, p.y, p.z );
	        		pushMatrix();
	        		translate( p.x, p.y, p.z*0.5f );
	            	fill( 204+p.z*0.5f, 102+p.z*0.5f, p.z*0.5f );
	        		box( 3, 3, p.z );
	        		popMatrix();
